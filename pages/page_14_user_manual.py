...
if section == "1. 介绍":
    ...
    3. **注意事项：**  
        1. 虽然grounding Google Search，依然会有幻觉现象产生，所以出发地和目的地尽量选择航线枢纽城市，如北京，上海，纽约，伦敦之类的城市。  
    ...
    st.markdown("""
        ...
        3. **注意事项：**  
        1. 因为这两个应用使用的是同一个项目，而我使用的是直接集成DialogFlow应用的方式，而不是通过SDK方式开发的，所以如果在同一个页面上切换两个应用时，例如从“游戏客服平台”直接切换至“电商客服平台”（或者相反）时，聊天窗口内会保留前一个客服平台的对话记录，为了防止这种事情发生，我在代码中特意做了“在新浏览器标签页中打开”的逻辑，即，当用户从“游戏客服平台”切换至“电商客服平台”（或相反）时，会新打开一个浏览器标签页。而从本应用内的任何其他页面进入这两个应用其中之一时，则不会如此。  
